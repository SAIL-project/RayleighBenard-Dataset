...
from fluiddata.config.dataclass.rbc_sim_config import RBCSimConfig
from fluiddata.dataset import RBCField
from fluiddata.simulation.rbc.shenfun.rayleighbenard2d import RayleighBenard, sympy
from fluiddata.simulation.rbc.shenfun.tfunc import Tfunc
from fluiddata.utils import VectorFieldVisualizer
...
class RayleighBenardEnv(gym.Env[RBCAction, RBCObservation]):
    ...
    def __init__(
        ...
        if render_mode == "live":
            self.window = VectorFieldVisualizer(
                size=sim_cfg.N,
                vmin=sim_cfg.bcT[1],
