...
from rbcdata.callbacks.callbacks_sb import LogNusseltCallback
...
def main(cfg: DictConfig) -> None:
    ...
    eval_env = make_vec_env(
        lambda: RayleighBenardEnv(cfg, nusselt_logging=True),
    ...
    checkpoint_callback_training = CheckpointCallback(
        ...
        * int(cfg.sim.episode_length / cfg.action_duration),
    ...
    callbacks = [
        LogNusseltCallback(
            cfg.sb3.ppo.episodes_update * int(cfg.sim.episode_length / cfg.action_duration)
        ),
        eval_callback,
    ...
    # start = time.time()
    model.learn(total_timesteps=cfg.sb3.train_steps, progress_bar=False, callback=callbacks)
