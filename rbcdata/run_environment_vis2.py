...
def update_vis(frame, window: RBCFieldVisualizer, sequence):
    ...
    window.draw(
        sequence[frame, RBCField.T],
        sequence[frame, RBCField.UX],
        sequence[frame, RBCField.UY],
        frame * 1,
    )
...
def run_env(cfg: DictConfig) -> None:
    ...
    ani = animation.FuncAnimation(
        fig=window.fig,
        func=partial(update_vis, window=window, sequence=observed_steps),
        frames=len(observed_steps),
        interval=500,
    )
    ...
    ani2 = animation.FuncAnimation(
        fig=window_convection.fig,
        func=partial(
            update_vis_convec, window=window_convection, sequence=observed_steps_convection
        ),
        frames=len(observed_steps_convection),
        interval=500,
    )
    ...
    ax_nusselt.set_ylim(
        [
            np.min(nusselt_nrs) - 0.1 * diffmaxMinNusselt,
            np.max(nusselt_nrs) + 0.1 * diffmaxMinNusselt,
        ]
    )
    ...
    ani3 = animation.FuncAnimation(
        fig_nusselt,
        func=partial(update_vis_nusselt, line=line, nusselt_nrs=nusselt_nrs),
        frames=end,
        interval=500,
        blit=True,
    )
