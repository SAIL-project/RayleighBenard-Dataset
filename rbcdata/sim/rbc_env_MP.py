...
class RayleighBenardEnvMP(RayleighBenardEnv):
    """
    ...
    def __init__(
        ...
        self.action_segments = 10   # Points in the domain where the agent can act
        self.action_start = action_start

        # The agent takes actions between [-1, 1] on the bottom segments
        # Note that shape=(1, ), because we learn one agent controlling one bottom segment
        self.action_space = gym.spaces.Box(-1, 1, shape=(1,), dtype=np.float32)

        # Observation Space (TODO may add probes for comparison with Vignon)
    ...
    def reset(
        ...
        self.t, self.tstep = self.simulation.initialize(
            filename=filename, np_random=self._np_random, rand=0.001
        )
    ...
    def get_obs(self, idx_seg) -> RBCObservation:
        """
        Function to return the observation of the environment centered at idx_seg
        """
        if idx_seg in [1, 3, 4, 5, 7, 9]:
            sizeSegment = 10
        else:
            sizeSegment = 9
        self.get_state[]
        self.simulation.outputs() 
        self.simulation.obs
