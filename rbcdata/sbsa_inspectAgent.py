...
from rbcdata.sim.rbc_env import (  # the environment that the single agent will interact with
    RayleighBenardEnv,
)
...
parser = argparse.ArgumentParser(
    description="Inspect a trained agent in the Rayleigh-Benard environment"
)
parser.add_argument(
    "--experiment_dir",
    type=str,
    help="Directory of the experiment with the trained policy and the used configuration.",
)
parser.add_argument(
    "--apply_policy",
    type=bool,
    default=True,
    help="Whether to apply the policy or just look at constant case.",
)
...
# TODO remove this, if save_checkpoint=False RayleighBenardEnv should not expect an output dir
...
# policy_SA =
#   PPO.load(join(experiment_dir, 'model_checkpoint_train/PPOmodelRBC_16000_steps'), env=test_env)
policy_SA = PPO.load(join(experiment_dir, "PPOmodelRBC_besteval_40000_steps"), env=test_env)

visCallback = RBCVisCallback(
    size=config.sim.N, bcT=config.sim.bcT, action_limit=config.action_limit, interval=1
)
...
for i in range(evalfortraindurations * int(config.sim.episode_length / config.action_duration)):
    if apply_policy:
        action = policy_SA.predict(
            obs, deterministic=True
        )  # returns the action as well as the hidden state which we don't use here
        action = action[0]
        print(action)
    else:
    ...
    # logger.info(f"Step {i}, after applying action: reward={reward},
    #   Nusselt={env.simulation.compute_nusselt(False)}, closed={closed},
