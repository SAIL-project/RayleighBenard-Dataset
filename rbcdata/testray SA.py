...
def run_env(cfg: DictConfig) -> None:
    ...
    # An RLlib environment normally consists of, but in this case it is just a Gym environment that also consists of: action space (all possible actions), state space
    ...
    # TODO for comparison with the Multi-Agent case, we could decide to also work with probes, which are used to collect data from the environment at certain points in space.
    ...
    # register_env("RayleighBenardEnv", lambda config: RayleighBenardEnv())

    # reset the environment to the initial state
    obs, info = env.reset() # TODO maybe load a checkpoint here from the created checkpoints.
    ...
    config = config.env_runners(num_env_runners=1) # for simplicity first we use 1, later on should be more parallel using more workers for the rollouts.
    config = config.environment(env)    # here I set the environment for where this policy acts in
