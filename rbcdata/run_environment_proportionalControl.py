...
# In this environment I am applying a proportional control between the cells, in order to see if it will break the two-cell setup.
# Strategy, measure the temperature in the middle row of the simulation, apply proportional control like in Tang and Bau using these values.
...
def run_env(cfg: DictConfig) -> None:
    ...
    while True:
        ...
        state = env.get_state()
        sensorT_midrow = state[-1, N[0] // 2 - 1, :]    # temperature sensor measurement of the middle row of the layer
        ...
        # At this point: Internal model of the environment used for planning the next action vs. no internal model of the environment used for planning the next action (Model-based vs. model-free)
        _, _, terminated, truncated, _ = env.step(action_heaters)   # perform the action in the environment
